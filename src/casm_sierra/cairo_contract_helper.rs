...
use crate::casm_sierra::contract_segmentation::{
...
pub enum StarknetSierraCompilationError {
    ...
    UnsupportedSierraVersion {
        version_in_contract: VersionId,
        version_of_compiler: VersionId,
    },
    #[error("Failed to compile sierra to casm")]
    FailedToCompileSierraToCasm,
}
...
impl CasmContractClass {
    ...
    pub fn get_bytecode_segment_lengths(&self) -> NestedIntList {
        self.bytecode_segment_lengths
            .clone()
            .unwrap_or(NestedIntList::Leaf(self.bytecode.len()))
    }
    ...
    match node {
        ...
        NestedIntList::Node(nodes) => {
            ...
            let inner_nodes = nodes
                .iter()
                .map(|node| bytecode_hash_node(iter, node))
                .collect_vec();
...
impl CasmContractClass {
    ...
    pub fn from_contract_class(
    ...
    ) -> Result<SierraContractCompile, StarknetSierraCompilationError> {
        ...
        for entry_points in [
            ...
            for (prev, next) in entry_points.iter().tuple_windows() {
                match prev.selector.cmp(&next.selector) {
                    ...
                    Ordering::Equal => {
                        return Err(
                            StarknetSierraCompilationError::DuplicateEntryPointSelector {
        ...
        for index in entrypoint_function_indices.clone() {
            ...
            *usages += 1;
            ...
            if *usages > MAX_SIERRA_FUNCTION_USAGES {
                return Err(
                    StarknetSierraCompilationError::DuplicateEntryPointSierraFunction { index },
                );
            }
        }
        ...
        let bytecode_segment_lengths =
            if sierra_version.minor >= CONTRACT_SEGMENTATION_MINOR_VERSION {
                Some(compute_bytecode_segment_lengths(
                    &program,
                    &cairo_program,
                    bytecode.len(),
                )?)
        ...
        {
            Ok(SierraContractCompile {
                casm_sierra_mapping_instruction,
                casm_contract_class: Self {
                    prime,
                    compiler_version,
                    bytecode,
            ...
            return Err(StarknetSierraCompilationError::FailedToCompileSierraToCasm);
        }
    }
}
